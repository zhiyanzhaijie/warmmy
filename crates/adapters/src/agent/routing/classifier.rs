...
const ROUTER_PREAMBLE: &str = r#"你是 warmmy 的对话路由器。

把用户本轮输入分类为且只分类为以下类别之一：
- meal_intake: 用户正在表达一次真实摄入、吃了/喝了具体食物、给出食物短句可能是在记录饮食、要求创建待确认用餐记录，确认/取消待确认用餐记录，或本轮图片很可能用于识别用餐。也包括用户在追问、纠正或投诉上一轮用餐记录流程没有生成待确认记录、没有落库、没有看到记录/卡片，要求你重新检查或继续处理刚才那顿饭；这类输入即使没有再次写出具体食物，也属于 meal_intake。
- chat: 普通聊天、营养知识咨询、泛泛建议、没有具体摄入事实的问题，或只是讨论食物但不是记录自己这次吃喝。

判断准则：
- 如果本轮语义依赖上一轮 meal 记录流程的状态、失败、补救、确认或取消，选择 meal_intake。
- 只有当用户完全没有在处理一条用餐记录，也没有在追问记录状态时，才选择 chat。

只输出一个小写英文标签：meal_intake 或 chat。
...
pub async fn classify_route(
    ...
    let rule_route = classify_by_rules(input, has_images);
    if rule_route != AgentRoute::Chat {
        return Ok(RouteDecision {
            route: rule_route,
            source: RouteDecisionSource::Rules,
            reason: format!("{rule_route:?} rule matched"),
        });
    }

    if input.trim().is_empty() && !has_images {
        return Ok(RouteDecision {
    ...
    let prompt = build_router_prompt(input, has_images);
    let raw = prompt_route_model(model, prompt).await?;
    let route = parse_route(&raw).unwrap_or_else(|| {
        if has_images {
    ...
    let source = if parse_route(&raw).is_some() {
...
async fn prompt_route_model(model: &ResolvedAIModelConfig, prompt: String) -> AppResult<String> {
    match model.provider.as_str() {
        ...
        provider => Err(AppError::internal(format!(
            "unsupported route classifier provider: {provider}"
        ))),
    }
}

fn parse_route(raw: &str) -> Option<AgentRoute> {
    let normalized = raw
        .trim()
