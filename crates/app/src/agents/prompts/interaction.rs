use crate::conversation::AgentInteractionContinuation;

const INTERNAL_CONTINUATION_MARKER: &str = "[warmmy:internal-continuation]";

pub fn interaction_continuation_prompt(interaction: AgentInteractionContinuation) -> String {
    match interaction {
        AgentInteractionContinuation::ConfirmMealLog { pending_id } => format!(
            "{INTERNAL_CONTINUATION_MARKER} 用户已在界面确认一条待确认用餐记录。请立即调用 confirm_meal_log 工具正式保存，不要重新询问。pending_id: {pending_id}。只有工具成功返回后，才能告诉用户已经记录，并基于记录给出简短说明。"
        ),
